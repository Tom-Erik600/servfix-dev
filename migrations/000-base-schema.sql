-- =============================================================================
-- ServFix — Grunnleggende tenant-schema
-- =============================================================================
-- Basert på database/create_tables.sql fra servfix-dev (snapshot 2026-05-04),
-- med tillegg av de to DB-funksjonene som ikke var med i den originale filen.
--
-- Idempotent: bruker IF NOT EXISTS og DO $$ BEGIN ... END $$ overalt.
-- Safe å kjøre mot eksisterende databaser — dropper/trunkerer ingenting.
--
-- Kjøres mot en ny, tom tenant-database FØR migrasjonene 001–008.
--
-- Bruk (via Cloud SQL Proxy på port 5433):
--   psql -h localhost -p 5433 -U postgres -d varing_db \
--        -f migrations/000-base-schema.sql
-- =============================================================================

-- =============================================================================
-- Functions (må være før views som bruker dem)
-- =============================================================================
...
CREATE OR REPLACE FUNCTION format_avvik_number(avvik_num INTEGER)
RETURNS TEXT
LANGUAGE plpgsql AS $$
BEGIN
    RETURN LPAD(avvik_num::TEXT, 3, '0');
END;
$$;

CREATE OR REPLACE FUNCTION get_next_avvik_number(report_id VARCHAR)
RETURNS INTEGER
LANGUAGE plpgsql AS $$
DECLARE
    next_number INTEGER;
BEGIN
    ...
    SET avvik_counter = avvik_counter + 1
    WHERE id = report_id
    RETURNING avvik_counter INTO next_number;
...
-- tenant_integrations_id_seq removed: table moved to servfix_admin (migration 002)
...
-- tenant_integrations table removed from per-tenant schema.
-- It now lives in servfix_admin and is created by migrations/seeds/000-tenant-integrations-admin.sql.
-- See migration 002-move-tenant-integrations-to-admin.js.
...
-- idx_integrations_tenant_provider removed: tenant_integrations moved to servfix_admin (migration 002).
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'update_instructions_updated_at' AND event_object_table = 'checklist_instructions') THEN
    CREATE TRIGGER update_instructions_updated_at
      BEFORE UPDATE ON checklist_instructions
      FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'trigger_equipment_updated_at' AND event_object_table = 'equipment') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'update_service_reports_updated_at' AND event_object_table = 'service_reports') THEN
